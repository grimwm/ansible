...
| `windows-dev` | yes | Windows only: installs dev tooling via winget (Windows Terminal, Git, VS Code, Docker Desktop, Postman, 7-Zip, Notepad++), creates `Documents\Repositories` and `Documents\WSL`, and deploys `Watch-OutlookGraph.ps1` to `%USERPROFILE%\outlook-graph-monitor`. |
...
`Watch-OutlookGraph.ps1` to `%USERPROFILE%\outlook-graph-monitor\`. This
PowerShell script polls Microsoft Graph for Outlook mail and dispatches messages
to a webhook (e.g. n8n) for processing.

The script is deployed only — it is **not** installed as a scheduled task or
service. Run it manually or set up your own scheduler. Graph authentication and
webhook configuration are outside the scope of this playbook; see the script's
built-in help (`Get-Help .\Watch-OutlookGraph.ps1 -Full`) for usage.
